...
class SEARCH_ALGS(MAZE):

    def heuristic(self,p1,p2,type):
        if type == 0: # manhattan
    ...
    def Astar(self, pos_start = (0,0), pos_end = (0,0), visualize = True):
        ...
        while pos_end != pos_cur[0] and openlist:
            time.sleep(.05)
            openlist.sort(key = (lambda x: x[2] + x[1]), reverse=True) # we sotrt according to heuristic + distance traveled so far
